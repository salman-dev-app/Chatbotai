...
async def send_long_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, text: str):
    MAX_LENGTH = 4000  # আপনার অনুরোধ অনুযায়ী পরিবর্তিত
...
async def ask_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_long_message(context, chat.id, response.text)
    except Exception as e:
...
async def handle_private_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not response.parts:
            await update.message.reply_text(get_text(user_id, 'safety_block')); return
        await send_long_message(context, chat_id, response.text)
    except Exception as e:
...
def main():
    ...
    logger.info("Bot starting with final, stable code and message splitting logic...")
